...
export async function GET(request: Request) {
  ...
  try {
    ...
    // 1. Публикации (посты пользователя)
    ...
    const thanksCount = reactions?.filter(r => 
      r.type === "thanks" || r.type === "like" || r.type === "❤️"
    ).length || 0;
    ...
    const { data: deals } = await supabase
      ...
      .eq("seller_id", userId)
      .or(`buyer_id.eq.${userId},status.eq.completed`);

    const dealsCount = deals?.length || 0;
    const turnover = deals?.reduce((sum, deal) => 
    ...
    const totalRep = repData?.reduce((sum, r) => sum + (r.score || 0), 0) || 0;
    ...
    const available = accounts?.find((a) => a.type === "available")?.balance ?? 0;
