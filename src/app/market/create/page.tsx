    ...
    <div className="citadel-container py-6 max-w-lg mx-auto space-y-4">
      ...
      <Card padding="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Подробное описание..." rows={5} className="w-full rounded-control bg-surface border border-line-subtle p-3 text-sm text-ink placeholder:text-ink-faint focus:outline-none focus:border-gold-400 resize-none" required />
          ...
          <div className="flex gap-2">
            {["service", "product", "digital"].map((t) => (
              <button key={t} type="button" onClick={() => setType(t)} className={`px-4 py-2 rounded-control text-sm font-medium transition-colors ${type === t ? "bg-gold-500 text-black" : "bg-surface text-ink-muted hover:bg-surface-2"}`}>{t === "service" ? "Услуга" : t === "product" ? "Товар" : "Цифровой"}</button>
            ))}
          </div>
